...
class ImageTrainerTFRay:
    ...
    @staticmethod
    def build_and_train(model_class, train_data, test_data, config):
        ...
        trainer = TensorflowTrainer(
            train_loop_per_worker=ImageTrainerTFRay.train_loop_worker_func,
    ...
    @staticmethod
    def train_loop_worker_func(config):
        ...
        with strategy.scope():
            ...
            train_data = ImageTrainerTFRay.to_tf_dataset(dataset=train_data, batch_size=batch_size)
            test_data = ImageTrainerTFRay.to_tf_dataset(dataset=test_data, batch_size=batch_size)

            # history = model.fit(train_data, validation_data=test_data, epochs=epochs, callbacks=[ReportCheckpointCallback()])
