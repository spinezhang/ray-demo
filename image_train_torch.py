...
class ImageTrainerTorch(ImageTorchProcess):
    ...
    def train(self, num_epochs):
        ...
        self.prepare_model()
        optimizer = Adam(self.net.parameters(), lr=0.001, weight_decay=0.0001)
        # optimizer = torch.optim.SGD(self.net.parameters(), lr=1e-3)
        scheduler = LambdaLR(optimizer, lr_lambda=lambda x: 1 / (x + 1))
        ...
        for epoch in range(num_epochs):
            ...
            self.net.eval()
            test_acc = self.test(self.net, self.test_data, self.test_len)
...
class ImageTrainerTorchSingle(ImageTrainerTorch):
    def __init__(self, net, train_data, test_data, config):
        super(ImageTrainerTorchSingle, self).__init__(net, config['use_gpu'])
    ...
    def save_checkpoint(self, epoch, train_acc, train_loss, test_acc):
        if test_acc > self.test_acc:
            torch.save(self.net.state_dict(), "cifar10_torch_single.model")
...
class ImageTorchInferenceSingle(ImageTorchProcess):
    ...
    def batch_predict(self, test_dataset):
        ...
        return self.test(self.net, test_data, length)
